...
class MoveBytecodeDecompiler: BinaryFileDecompiler {
...
//        val project =
//            ProjectLocator.getInstance().getProjectsForFile(file).firstOrNull { it?.isEndlessConfigured == true }
//                ?: ProjectManager.getInstance().defaultProject.takeIf { it.isEndlessConfigured }
...
//        return LoadTextUtil.loadText(targetFile)
    ...
    fun decompileFileToTheSameDir(endless: Endless, file: VirtualFile): RsResult<VirtualFile, String> {
        endless.decompileFile(file.path, outputDir = null)
            .unwrapOrElse {
                return RsResult.Err("`endless move decompile` failed")
            }
    ...
    fun decompileFile(endless: Endless, file: VirtualFile, targetFileDir: Path): RsResult<VirtualFile, String> {
        if (!targetFileDir.exists()) {
            targetFileDir.toFile().mkdirs()
        }

        endless.decompileFile(file.path, outputDir = targetFileDir.toString())
            .unwrapOrElse {
                return RsResult.Err("`endless move decompile` failed")
            }
