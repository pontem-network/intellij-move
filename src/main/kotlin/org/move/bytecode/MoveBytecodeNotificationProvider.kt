...
import org.move.cli.settings.getEndlessCliDisposedOnFileChange
...
class MoveBytecodeNotificationProvider(project: Project): EditorNotificationProvider {
    ...
    override fun collectNotificationData(
        ...
        if (!FileTypeRegistry.getInstance().isFileOfType(file, MoveBytecodeFileType)) {
            return null
        }
        val properties = PropertiesComponent.getInstance(project)
        val decompilationFailedKey = DECOMPILATION_FAILED + "." + file.path

        val endlessDecompiler = MoveBytecodeDecompiler()
        val decompiledFilePath = file.parent.pathAsPath.resolve(endlessDecompiler.sourceFileName(file))

        // null if no Endless configured
    ...
    class DecompilationModalTask private constructor(
        ...
        override fun compute(indicator: ProgressIndicator): RsResult<VirtualFile, String> {
            return MoveBytecodeDecompiler().decompileFileToTheSameDir(endless, file)
        }

        fun runWithProgress(): RsResult<VirtualFile, String> = ProgressManager.getInstance().run(this)

        companion object {
            /// fails if Endless CLI is not configured
            fun forVirtualFile(project: Project, file: VirtualFile): DecompilationModalTask? {
                ...
                val endless =
                    project.getEndlessCliDisposedOnFileChange(file) ?: return null
                return DecompilationModalTask(project, endless, file)
            }
        }
    }

    companion object {
        private const val DECOMPILATION_FAILED = "org.move.endlessDecompilerNotificationKey"
