...
class PerProjectEndlessConfigurable(val project: Project): BoundConfigurable("Endless") {

    private val extraTextArgsField = ExpandableTextField()

    override fun createPanel(): DialogPanel {
        val chooseEndlessCliPanel = ChooseEndlessCliPanel(versionUpdateListener = null)
        this.disposable?.let {
            Disposer.register(it, chooseEndlessCliPanel)
        }
        return panel {
            ...
            group {
                row {
                    checkBox("Fetch external packages on project reload")
                        .bindSelected(state::fetchEndlessDeps)
                ...
                group("Extra CLI arguments") {
                    row {
                        fullWidthCell(extraTextArgsField)
                            .resizableColumn()
                            .comment(
                                "Additional arguments to pass to <b>endless move test</b>"
                            )
                            .bind(
                    ...
                    row {
                        checkBox("Disable telemetry for new Run Configurations")
                            .comment(
                                "Adds ENDLESS_DISABLE_TELEMETRY=true to every generated Endless command."
                            )
            ...
            if (!project.isDefault) {
                row {
                    link("Set default project settings") {
                        ProjectManager.getInstance().defaultProject.showSettingsDialog<PerProjectEndlessConfigurable>()
                    }
                        .align(AlignX.RIGHT)
                }
            }
            ...
            onApply {
                settings.modify {
                    val localEndlessSdkPath = chooseEndlessCliPanel.data.endlessPath
                    if (localEndlessSdkPath != null) {
                        chooseEndlessCliPanel.updateEndlessSdks(localEndlessSdkPath)
                    }
                    it.endlessPath = localEndlessSdkPath

                    it.disableTelemetry = state.disableTelemetry
                    it.extraTestArgs = state.extraTestArgs
                    it.enableMove2 = state.enableMove2
                    it.fetchEndlessDeps = state.fetchEndlessDeps
                }
            }

            // loads settings from configurable to swing form
            onReset {
                chooseEndlessCliPanel.data = ChooseEndlessCliPanel.Data(state.endlessPath)
            }

            /// checks whether any settings are modified (should be fast)
            onIsModified {
                val endlessPanelData = chooseEndlessCliPanel.data
                endlessPanelData.endlessPath != settings.endlessPath
            }
        }
    }
}
